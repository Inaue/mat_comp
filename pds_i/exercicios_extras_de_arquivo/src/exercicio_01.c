...
#define CHARS_ARQUIVO        (32 + CHAR_FIM_DE_STR)
...
int main()
{
	...
	char nome_da_entrada[CHARS_ARQUIVO], nome_da_saida[CHARS_ARQUIVO];
	char * leitura_do_registro;
	FILE * entrada, * saida;

	printf("Digite o nome do arquivo de entrada:\n");
	fgets(nome_da_entrada, CHARS_ARQUIVO, stdin);
	remova_quebra_de_linha(nome_da_entrada);
	printf("Digite o nome do arquivo de saida:\n");
	fgets(nome_da_saida, CHARS_ARQUIVO, stdin);
	remova_quebra_de_linha(nome_da_saida);
	entrada = fopen(nome_da_entrada, "r");
	saida   = fopen(nome_da_saida,   "w");
	le_cabecalho(entrada, cabecalho, " ");

	if(entrada == NULL || saida == NULL)
	{
		perror("ERRO: algum dos arquivos nao pode ser aberto!\n");
		
		return ARQUIVO_NAO_ABERTO;
	}

	while(VERDADEIRO)
	{
		leitura_do_registro = fgets(registro, CHARS_REGISTRO, entrada);
		
		if(leitura_do_registro == NULL)
			break;

		remova_quebra_de_linha(registro);
		tabule_registro(saida, cabecalho, registro, ";");
	}
